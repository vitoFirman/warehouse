...
import { stockIn as fetchStockIn } from "../../../Redux/Features/stockSlice";
import DatePicker from "react-datepicker"
import {id as LocaleId} from 'date-fns/locale'

const ModalEditStockIn = ({id}) => {
    const dispatch = useDispatch()
    const open = useSelector(state => state.modal.open)
    const stockIns = useSelector(state => state.stock.stockIn.data)
    const stockIn = stockIns.find((stockIn) => stockIn.id === id)
    ...
    useEffect(() => {
        if(stockIn) {
            setFormData({
                ...
                supplier_name: stockIn.supplier.name,
                qty: stockIn.qty,
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const res = await axios.put(`${baseUrl}/stock/in/update/${id}`, formData, {
                headers: {
...
export default ModalEditStockIn

ModalEditStockIn.propTypes = {
    id: PropTypes.number
}